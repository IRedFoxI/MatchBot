	...
	module IRC
		module Events
			module Numerics
				...
				class WhoWasUserNumeric < Kesh::IRC::Events::NumericEvent
				
					def WhoWasUserNumeric.parse( server, source, id, target, tokens )
						return nil unless (
							tokens.length == 5 &&
							id == RPL_WHOWASUSER
						)						
						
						return WhoWasUserNumeric.new( server, source, id, target, server.getClientByName( tokens[ 0 ] ), tokens[ 1 ], tokens[ 2 ], tokens[ 4 ] )
					end
